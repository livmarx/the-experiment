...
<template>
  <div>
    ...
    <form @submit.prevent>
      ...
      <TextInput label="Start Time" :inputData="trial.startTime" @emitData="updateLocalData($event, 'startTime')"/>
      <TextInput label="End Time" :inputData="trial.endTime" @emitData="updateLocalData($event, 'endTime')"/>
      <TextInput label="Subject ID" :inputData="trial.subjectId"  @emitData="updateLocalData($event, 'subjectId')"/>
      <TextInput label="Trial Number" :inputData="trial.trialNumber" @emitData="updateLocalData($event, 'trialNumber')"/>
      ...
      />
      <button @click="createTrial">Save Trail Data</button>
    </form>
  </div>
</template>

<script>
import { createDocument } from '@/apis/databaseApi.js';
...
export default {
  ...
  methods: {
    createTrial() {
      createDocument('trials', this.trial)
        .then(() => {
